...
/*
如果是在 main 函数之后进行函数定义的，那么调用时就一定要在 main 函数之前进行声明，
如果在自定义函数在 main 函数之前定义的，就可以不需要进行声明，main 函数能够自动找到该函数。
如果不进行函数声明，在其他的编译器中就会出现自定义函数找不到的现象
*/

   // system("pause");
// 计算工具.cpp : 此文件包含 "main"
// 函数。程序执行将在此处开始并结束。
// 先让user判断有无分数，还要解决a为不为0的情况，a为零的时候分母是无意义的；
// 根式还要继续化简，提数出来
